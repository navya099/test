...
class SegmentVisualizer(tk.Tk):
    ...
    def __init__(self, alignment):
        ...
        # 곡선 변경 버튼
        self.change_curve_button = ttk.Button(control, text="곡선 변경", command=self.update_radius_ui)
        self.change_curve_button.pack(side=tk.LEFT, padx=10)
    ...
    def update_radius_ui(self):
        ...
        try:
            ...
            # --- 반경 입력 대화상자 ---
            radius_str = simpledialog.askstring("곡선 변경", "곡선 반경 (R)을 입력하세요:", parent=self)
            if not radius_str:
                return  # 사용자가 취소 누름
            try:
                radius = float(radius_str)
            ...
            self.update_plot()
            self.json_export()
            messagebox.showinfo("완료", f"PI {idx}의 반경을 {radius:.2f}m 로 변경했습니다.")
    ...
    def _draw_segments(self):
        ...
        #pi 번호
        for i, pt in enumerate(self.collection.coord_list):
            if i ==0:
                self.ax.text(pt.x, pt.y, f'BP')
            elif i == len(self.collection.coord_list)-1:
                self.ax.text(pt.x, pt.y, f'EP')
            else:
                self.ax.text(pt.x, pt.y,f'IP.NO{i}')
        self.ax.axis("equal")
