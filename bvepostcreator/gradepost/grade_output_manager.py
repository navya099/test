...
class GradeOutputManager(KMOutputManager):

    def create_pitch_post_txt(self, data_list: list[VIPObjectDATA]):
        """
        ...
        for data in data_list:
            ...
            if self.track_mode == 'single':
                text = f"{data.station},.freeobj {main_track_idx};{data.object_index};{main_x};{data.offset[1]};{main_rotation};,;VIP{data.no}_{data.vcurve_type}-{data.structure}\n"  # 원하는 형식으로 저장
                texts.append(text)
                ...
                text1 = f"{data.station},.freeobj {self.track_index['main']};{data.object_index};{main_x};{data.offset[1]};{main_rotation};,;VIP{data.no}하_{data.vcurve_type}-{data.structure}\n"  # 원하는 형식으로 저장
                text2 = f"{data.station},.freeobj {self.track_index['sub']};{data.object_index};{sub_x};{data.offset[1]};{sub_rotation};,;VIP{data.no}상_{data.vcurve_type}-{data.structure}\n"  # 원하는 형식으로 저장
                texts.append(text1)
                texts.append(text2)
