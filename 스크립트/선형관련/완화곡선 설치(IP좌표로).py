...
def determine_curve_type(d10, d11, l11):
    '''
    # Example usage
    d10 = R
    d11 = M
    l11 = V
    '''
    if d10 > 0 and d11 > 0 and l11 > 0:
        result = 1  # Assuming "3차 포물선" means "3rd parabola" in Korean
    else:
        result = 0  # Assuming "단곡선" means "elliptical curve" in Korean
    return result
...
def calculate_spiral(design_speed,direction,BP_IP_BREAING,IP_EP_BREAING,BP_XY,EP_XY,R,IP_XY,IA):
    ...
    is_cubicparabola = determine_curve_type(R,m,v_ds)#1ㅇ이면 3차포물선 아니면 원구ㅗ선
    ...
    print('IP_EP_BREAING',IP_EP_BREAING)
...
def Create_KML(point1,array1,array2,point2,acr1,acr2):# Create a KML object
