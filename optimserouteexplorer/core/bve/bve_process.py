...
class BVEProcess:
    ...
    def save_infos(self):
        ...
        vec_list = [Vector3(x=c[0], y=c[1], z=0.0) for c in coords]  # z는 필요시 추가
        BVEInfoSaver.save_cooridnate_info('C:/Temp/bve_coordinates.txt', vec_list)

        # 5 구조물xlsx 저장
        bridges = self.results['result']['bridges']
        turnnels = self.results['result']['tunnels']
        structures = StructureCollection()
        structures.extend(bridges)  # 리스트 전체를 풀어서 추가
        structures.extend(turnnels)
