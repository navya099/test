...
def process_routes(start, end, n_candidates, n_generations ,chian):
    """GA 실행 및 결과 반환"""
    results = run_process(start, end, n_candidates, n_generations, chian)
    return results


def format_results(results, top_n):
    """상위 10개 결과 포맷팅"""
    return format_top10(results, top_n)
...
def visulalize(top10, start, end, top_n):
    visualize_routes_with_button(top10, start=start, end=end, top_n=top_n,
                                 map_file="c:/temp/candidate_routes.html")


def run_main_process(start, end, n_candidates, n_generations, top_n, chain):

    # 2. 원본 처리
    results = process_routes(start, end, n_candidates, n_generations, chain)

    # 3. 포맷팅
    top10 = format_results(results, top_n)

    # 4. 출력
    export_results(top10)

    #5 #시각화
    visulalize(top10, start, end, top_n)
