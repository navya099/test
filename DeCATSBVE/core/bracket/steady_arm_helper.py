...
class SteadyArmHelper:
    @staticmethod
    def get_index(dataprocessor, base_type, flip=False):
        steady_arm_dict = dataprocessor.get_steady_arm_fittings()

        # flip 적용: I ↔ O 반전
        if flip:
            base_type = 'O' if base_type == 'I' else 'I'

        return steady_arm_dict[base_type][0] if base_type == 'I' else steady_arm_dict[base_type][1]

    @staticmethod
    def create_fitting(pole, dataprocessor, idxlib, stagger, cw_height, rotation, flip=False):
        steady_arm_index = SteadyArmHelper.get_index(dataprocessor, pole.base_type, flip)
